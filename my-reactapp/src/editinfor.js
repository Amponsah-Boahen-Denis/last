...
function EditRecord() {
  ...
  const handleDelete = async (accountId) => {
    if (window.confirm('Are you sure you want to delete this account?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://lastback.vercel.app/account/${accountId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        setMessage('Account deleted successfully.');
        setAccounts(accounts.filter(account => account._id !== accountId)); // Remove account from the list
      } catch (error) {
  ...
  return (
    <div className='container'>
      ...
      {/* Accounts List with Delete Button */}
      <ul>
